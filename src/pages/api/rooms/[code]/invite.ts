...
export async function GET({ locals, params }: APIContext) {
  ...
  const invitations = (
    selectActiveInvitationsStatement.all(roomCode) as {
      id: string;
      inviteeId: string;
      inviteCode: string;
      createdAt: string;
    ...
    inviteLink: inviteLink(roomCode, row.inviteCode),
  }));
...
export async function POST({ locals, params, request }: APIContext) {
  ...
  const isTargetMember = checkMembershipStatement.get(roomCode, body.userId);
  if (isTargetMember) {
    return Response.json(
  ...
  return Response.json({
    invitation: {
      ...
      inviteCode,
      inviteLink: inviteLink(roomCode, inviteCode),
      expiresAt,
      status: "pending",
...
export async function DELETE({ locals, params, request }: APIContext) {
  ...
  const result = revokeInvitationStatement.run(body.invitationId, roomCode);
  if (result.changes === 0) {
